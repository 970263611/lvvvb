...
func createControlChannel() {
	tcpListener, err := utils.Listen(utils.ControlPort)
	if err != nil {
		panic(err)
	}

	log.Println("[已监听]" + strconv.Itoa(utils.ControlPort))
	for {
...
func keepAlive() {
	go func() {
		for {
			...
			_, err := clientConn.Write(([]byte)(utils.KeepAlive + "\n"))
			if err != nil {
...
func acceptUserRequest() {
	tcpListener, err := utils.Listen(utils.VisitPort)
	if err != nil {
		panic(err)
	}
	...
	for {
		...
		sendMessage(utils.NewConnection + "\n")
	}
}
...
func acceptClientRequest() {
	tcpListener, err := utils.Listen(utils.TunnelPort)
	if err != nil {
		panic(err)
	}
	defer tcpListener.Close()
...
func establishTunnel(tunnel *net.TCPConn) {
	...
	for key, connMatch := range connectionPool {
		if connMatch.accept != nil {
			go utils.Join2Conn(connMatch.accept, tunnel)
